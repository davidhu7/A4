...
app.post("/items",verify,(req,res,next)=>{
  let name = req.body.name
  Item.findOne({'name':name},function(err,result){
    if(err){
      res.status(500).send('500 Server error').send();}
    else{
      if(result){res.status(409).send("duplicate");}
      else{
        Item.create({'id':req.body.id, 'name':req.body.name, 'type':req.body.type,'img':req.body.img},function(err,result){
...
function searchItem(req,res,next){  
   ...
   let t = x.type
   let n = x.name
   Type.findOne({'name':t},function(err,temp){
     if(err)res.status(500).send('500 Server error');
     else{
       if(temp){
         let type = temp.id
         Item.find({'name':{"$regex":n,"$options":"i"},'type':type},function(err,result){
          if(err)res.status(500).send('500 Server error');
          else{
            res.json(result)
          }
